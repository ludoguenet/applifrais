...
function connecterServeurBD() {
    ...
    $login = "root";
    $mdp = "";
    // Ajouter un try and catch
    return new \PDO('mysql:host=' . $hote . ';dbname=gsb', $login, $mdp);
}
 ...
 * @return void  
 */
function deconnecterServeurBD($idCnx): void
{
    // mysql_close($idCnx);
}
...
function obtenirDetailVisiteur($idCnx, $unId) {
    ...
    $statement = $pdo->prepare("select id, nom, prenom from visiteur where id = ?");
    $statement->execute([$unId]);
...
function existeFicheFrais($idCnx, $unMois, $unIdVisiteur) {
    $pdo = connecterServeurBD();
    $statement = $pdo->prepare('select idVisiteur from FicheFrais where idVisiteur = ? and mois = ?');
    $statement->execute([$unIdVisiteur, $unMois]);
...
function obtenirDernierMoisSaisi($idCnx, $unIdVisiteur) {
    $pdo = connecterServeurBD();
    $statement = $pdo->prepare('select max(mois) as dernierMois from FicheFrais where idVisiteur = ?');
    $statement->execute([$unIdVisiteur]);
    $lastMonth = $statement->fetch();
...
function verifierInfosConnexion($idCnx, $unLogin, $unMdp) {
    ...
    $statement = $pdo->prepare("select id, nom, prenom, login, mdp from Visiteur where login = ? and mdp = ?");
    $statement->execute([$unLogin, $unMdp]);
    $user = $statement->fetch();
